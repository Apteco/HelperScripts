...
Function ImportCsv-ToSqlite {
    ...
    $results = ".mode csv",".separator $( $separator )",".import '$( $sourceCsv.Replace("\", "/") )' ""$( $destinationTable )""" | & $sqliteExe $sqliteDb.Replace("\", "/")
...
Function Sanitize-SqliteTableName {
    param(
    ...
    # Dashes cause problems in tablenames and are removed in this case
    $newTableName = $newTableName.Replace('-','')
    
    # Spaces are allowed but cause problems in the sqlite cli
    # in this case they are getting replaced with an underscore
    $newTableName = $newTableName.Replace(" ","_")

    # TODO [ ] Implement regex to remove prefix numbers
    # sqlite table names are not allowed to contain dashes or begin with numbers
