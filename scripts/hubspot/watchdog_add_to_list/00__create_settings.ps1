...
[ ] check if it is really sqlserver_v2.sh that gets triggered
...
# General settings
$modulename = "HUBSPOTCREATESETTINGS"
...
# Default file
$settingsFileDefault = ( Join-Path $scriptPath settings.json )
...
# Default file
$logfileDefault = ( Join-Path $scriptPath hubs.log )
...
Write-Log -message "Creating a new settings file" -severity WARNING
...
$accessToken = Read-Host -AsSecureString "Please enter the private apps token for hubspot"
$accessTokenEncrypted = Convert-PlaintextToSecure ((New-Object PSCredential "dummy",$accessToken).GetNetworkCredential().Password)
...
$settings = @{
    ...
    # General
    "base"="https://api.hubapi.com/"					# Default url
    "changeTLS" = $true                      	        # should tls be changed on the system?
    "providername" = "hubs"                           # identifier for this custom integration, this is used for the response allocation
    ...
    # Watchdog specific
    "watcher" = @{
        folderToWatch = "/root/go/payloads/sqlserver"     # The directory to watch
...
If ( Test-Path -Path $settingsFile ) {
    ...
    Write-Log -message "Moving previous settings file to $( $backupPath )" -severity WARNING
